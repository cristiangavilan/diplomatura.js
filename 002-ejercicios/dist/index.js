...
console.log('******************************************************************'); // 1) Importar el objeto 'database' del archivo "./basededatos"
...
var findUniversidadById = function findUniversidadById(id) {
  return _baseDeDatos.database.universidades.find(function (u) {
    return u.id === id;
  });
};
...
var findProfesorById = function findProfesorById(id) {
  return _baseDeDatos.database.profesores.find(function (p) {
    return p.id === id;
  });
};
...
console.log('******************************************************************'); // 4) Implementar una función que obtenga una materia por Id

console.log('4) Implementar una función que obtenga un profesor por Id');
console.log('');

var findMateriaById = function findMateriaById(id) {
  return _baseDeDatos.database.materias.find(function (m) {
    return m.id === id;
  });
};
...
console.log('******************************************************************'); // 🤓 Tip: Comparar con la función del ejercicio (3) y ver si se les ocurre una función genérica que sirva para cualquier tabla
